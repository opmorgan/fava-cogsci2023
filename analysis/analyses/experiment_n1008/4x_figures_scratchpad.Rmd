...
if (use_cached_rtfigs == F) {
  ...
  title = "Global bias asymmetry by EHI"
    ...
    quantile_data <- rt_subject_plot |> 
      mutate(quantile = as.factor(ntile(desc(ehi), 16)),
             bin = )
    
    temp <- rt_subject_plot
    
    ehi_vec <- temp$ehi
    dv_vec <- temp$dv
    
    ehi_vec |> cut_interval(16)
    ...
    quantile_data
    ## Calculate means and SEM by quantile
    ## Include each quantile's mean ehi value for plotting (x positions)
    quantile_data |> 
      group_by(quantile) |> 
      summarize(n = n(),
                mean_ehi = mean(ehi), ## Or: center at geometric intervals.
                mean_dv = )
