---
title: "Action Asymmetry Experiment (n = 1008): Demographics & Exclusions"
pagetitle: "aah | exp | exclusions"
author: "Owen Morgan"
...
fig_dir <- here::here("figures", "exp_n1008")
...
```{r}
## DEBUGGING
# aah_summary_all |> select(sample, sample2, prolific_handedness, handedness, ehi) |> 
#   group_by(sample2, prolific_handedness, handedness) |> 
#   summarize(n = n())
```
...
##   TODO: EHI handedness does not match pre-screener
...
## TODO: Mark exclusion if EHI response does not match pre-screened handedness
## Two ways to do this:
## (1) More conservative, probably too restrictive. Only include participants whose handedness matches our EHI bins: pre-screen left handers with EHI < -40, pre-screen ambis with ehi between -40 and 40, pre-screen right-handers with EHI > 40
## (2) Recommended -- more liberal -- just remove obvious mismatches. Remove pre-screen lefties with EHI > 0; Remove pre-screen ambis with EHI == 100 or -100; Remove pre-screen righties with handedness < 0.
aah_summary_all <- aah_summary_all |>
...
if (use_cached_demofigs == FALSE) {
  g <- ggplot(aah_summary_all, aes(x = ehi)) +
    ...
    labs(title = "EHI distribution for each of Prolific's prescreened handedness groups (all recruits with task data)")
...
if (use_cached_demofigs == FALSE) {
  g <- ggplot(aah_summary_nomismatches, aes(x = ehi)) +
    ...
    labs(title = "EHI distribution for each of Prolific's prescreened handedness groups (mismatches excluded)")
...
feedback <- aah_summary_all |>
  ...
  group_by(sample, open_ended_feedback_response, exclude) |>
  rename(`Open-ended feedback` = open_ended_feedback_response,
         `Excluded?` = exclude,
         Sample = sample) |> 
  summarize() |> 
  select(`Open-ended feedback`, `Excluded?`, Sample)
