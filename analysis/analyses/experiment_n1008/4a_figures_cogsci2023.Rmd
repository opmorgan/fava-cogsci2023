...
Dots show per-subject mnean, error bars show SEM.
...
if (use_cached_rtfigs == FALSE) {
  rt_subject_plot <- rt_subject |>
    ...
    g <- gg_rt_1_horizontal(title = "Global Bias Asymmetry by Handedness (EHI > +40 / < -40)",
...
if (use_cached_rtfigs == FALSE) {
  rt_subject_plot <- rt_subject |>
    ...
    g <- gg_rt_2_horizontal(title = "Hemifield bias for global and local targets, by handedness (EHI > +40 / < -40)",
                          rt_subject_plot,
                          handedness_labeller = NULL,
                          plot_colors = plot_colors
    )
    
  ggsave(fig_path_var, g, "png", height = 5, width = 8)
}
...
**In progress**. Dots show model-estimated group mean, error bars show 95% CI from model.
...
if (use_cached_rtfigs == FALSE) {
  ...
  g <- gg_rt_1_horizontal(title = "Global Bias Asymmetry by handedness (EHI = +/- 100), square targets only",
                          rt_subject_plot, plot_colors
    )
