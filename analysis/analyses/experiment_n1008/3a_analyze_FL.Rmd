...
## TODO: update these functions to load new aah_long, with prolific handedness column, ehi mismatch exclusion column.
...
sample_demo_summary_table(aah_summary) |> rename(`N (keepers)` = N) |>  pretty_table() |>
  tab_footnote("Left/mixedies: reported 'Left Handed' or 'Ambidextrous' on Prolific |  Righties: reported 'Right handed' on Prolific")
```
<br>
...
<br>
<br>
***Summary.*** We see the predicted effect, for both reaction time (28.35ms, 95%CI [13.54, 43.16], p < .001) and accuracy (OR = 1.66, 95%CI [1.22, 2.28], p = .001).
<br>
<br>
...
<br>
<br>
***Summary.*** We see the predicted effect, for both reaction time (27.27ms, 95%CI [17.17, 36.37], p < .001) and accuracy (OR = 2.00, 95%CI [1.62, 2.62], p < .001).
<br>
<br>
...
<br>
<br>
***Summary.*** We see the predicted effect, for both reaction time (27.63ms, 95%CI [19.28, 35.98], p < .001) and accuracy (OR = 1.89, 95%CI [1.56, 2.30], p < .001).
<br>
<br>
...
<br>
<br>
***Summary.*** We see the predicted effect, for both reaction time (27.28ms, 95%CI [19.62, 34.94], p < .001) and accuracy (OR = 1.96, 95%CI [1.65, 2.33], p < .001).
<br>
<br>
...
<br>
<br>
***Summary.*** We see a smaller effect in the same direction as right handers', for both reaction time (17.50ms, 95%CI [6.25, 28.75], p = .002) and accuracy (OR = 1.94, 95%CI [1.61, 2.35], p < .001).
<br>
<br>
```{r, child=c(here(analysis_dir, "components", "3a_analyze_FL_rt_EHI.Rmd"))}
```
