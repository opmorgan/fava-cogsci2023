...
pagetitle: "aah | exp | analysis (field by level by handedness)"
author: "Owen Morgan"
...
```
<br>
Using the bobyqa optimizer alone gives virtually the same results.
```{r}
acc_model_2bins_bobyqa <- readRDS(here(manual_cache_dir, "models_bobyqa", 
                                     "acc_model_2bins.rds"))
```
```{r, echo = T, results = "markup"}
...
if (use_cached_model_acc_ehi == FALSE) {
  ...
  ## I re-ran the model with the bobyqa estimate on Jan 7, 2023
  ## This gave virtually the same results
  ## (and still gave the warning about very large eigenvalue)
  # acc_model_ehi <-
  #   glmer(correct ~ field * level * ehi + (1 | subject),
  #         data = aah_for_acc_ehi_model,
  #         family = "binomial",
  ...
  ## To address large eigenvalue error, try rescaling continuous variables.
  ## continuous variables: EHI.
  # df <- as.data.frame(aah_for_acc_ehi_model)
  # numcols <- grep("ehi|rt", names(df))
  # df_scaled <- df
  # df_scaled[, numcols] <- scale(df_scaled[, numcols])
  # acc_model_ehi_scaled <- update(acc_model_ehi, data = df_scaled)
  # acc_model_ehi_scaled
  # summary(acc_model_ehi_scaled)
  # 
  # saveRDS(acc_model_ehi_scaled, here(manual_cache_dir,
  #                             "acc_model_ehi_scaled.rds"))
...
if (use_cached_model_acc_ehi == FALSE) {
  ...
  ## Do anova on scaled model
  # acc_model_no_interaction_scaled <-
  #   update(acc_model_ehi_scaled, . ~ . - field:level:ehi)
  # interaction_anova_scaled <-
  #   interaction_stats(acc_model_ehi_scaled, acc_model_no_interaction_scaled)
  # saveRDS(interaction_anova_scaled,
  #         here(manual_cache_dir,
  #              "acc_anova_ehi_scaled.rds"))
...
Using the bobyqa optimizer alone gives virtually the same results.
```{r}
acc_model_ehi_bobyqa <- readRDS(here(manual_cache_dir, "models_bobyqa", 
                                     "acc_model_ehi.rds"))
```
```{r, echo = T, results = "markup"}
...
```
<br>
Using a model with scaled EHI gives virtually the same results, and no warning about large eigenvalues.
```{r}
acc_model_ehi_scaled <- readRDS(here(manual_cache_dir, "models_scaled", 
                                     "acc_model_ehi_scaled.rds"))
```
```{r, echo = T, results = "markup"}
summary(acc_model_ehi_scaled)
