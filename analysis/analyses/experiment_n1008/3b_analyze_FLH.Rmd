...
<br>
<br>
 ***Summary.*** For reaction time, we find the critical interaction in the predicted direction (11.67ms, 95%CI [0.65, 22.69], p = .019, one-sided). I haven't run accuracy yet, but the effect will be close to zero, opposite the predicted direction (the point estimates are 1.76 for righties, 1.96 for lefties).
<br>
<br>
...
interaction_anova |>
  ...
  tab_footnote(footnote = "F-test (Two-sided?)",
...
```{r}
## Test whether interaction slope differs from zero
## How to do this???
# summary(rt_emm_ehi, infer=c(TRUE,TRUE),null=0)
# 
# rt_emm_ehi <- emmeans(rt_model_ehi, ~ field * level * ehi)
# summary(test(emtrends(mod, ~syst|timing, var = "year")))
# summary(test(emtrends(rt_model_ehi, ~field|level, var = "ehi")), infer = T)
```
...
$$
Each unit change in EHI (-100:100) corresponds to a **0.067ms** difference in LVF global bias. This is the slope estimate given by the summary function:
<br>
