...
rt_emt[[1]] |> contrast(interaction = c("consec"), var = "ehi") |> 
  ...
  tab_header(title = "Field by level interaction (RT)",
             subtitle = "Compare effect estimate to zero with emmeans()") |>
  tab_footnote(footnote = "A positive number means global bias is stronger in LVF for right handers (as predicted), in ms per EHI unit (-100 to 100). Multiply this value by 200 to get the estimated difference in LVF global bias for strong left vs. right handers.",
               locations = cells_column_labels(columns = estimate)) |>
  tab_footnote(footnote = "Two-sided",
               locations = cells_column_labels(columns = p.value)) |>
  tab_footnote(footnote = "Confidence level: 95%. Lower CI for one-sided test: 0.013.",
               locations = cells_column_labels(columns = ends_with("CL"))) |>
  tab_footnote(footnote = "Z-approximation",
               locations = cells_column_labels(columns = df))
```
...
L_tbl |> 
  format_p.value() |> 
  pretty_table() |> 
  tab_header(title = "Estimated global bias by field, for EHI of -100 (strong left hander)") |> 
...
acc_emt[[1]] |> contrast(interaction = c("consec"), var = "ehi") |> 
  ...
  format_p.value() |> 
  pretty_table() |> 
  tab_header(title = "Field by level interaction (acc)",
             subtitle = "Compare effect estimate to zero with emmeans()") |>
...
## Use ref_grid (following quant 2 HW5)
ref <- ref_grid(acc_model_ehi, at = list(ehi = c(-100, 100)))
#ref |> summary(infer = T) |> as_tibble() |> filter(ehi == -100)

ref_L <- ref_grid(acc_model_ehi, at = list(ehi = c(-100)))
ref_R <- ref_grid(acc_model_ehi, at = list(ehi = c(100)))

L_tbl <- ref_L |> contrast("pairwise", type = "response") |> as_tibble() |> slice(2, 5)
R_tbl <- ref_R |> contrast("pairwise", type = "response") |> as_tibble() |> slice(2, 5)
