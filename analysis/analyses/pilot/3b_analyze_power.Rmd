...
rt_model_sim_extended <- extend(rt_model_sim, along="subject", n=824)
...
pc <- powerCurve(fit = rt_model_sim_extended,
                  test = compare(rt ~ field*level + (1 | subject)),
                  alpha = 0.05,
                  along = "subject",
  ...
  saveRDS(rt_model_sim_extended, here(cache_dir, "rt_model_sim_extended.rds"))
  saveRDS(pc, here(cache_dir, "rt_model_sim_extended_pc.rds"))
} else if (use_cached_sims == TRUE) {
  pc <- readRDS(here(cache_dir, "rt_model_sim_extended_pc.rds"))
}

pc_tbl <- pc |> summary() |> as_tibble() |>
  rename(n_trials = nrow, n_subjects = nlevels,
         simulations = trials, power = mean)
pc_tbl |> 
