---
title: "Power analysis for preregistration (Reaction time)"
pagetitle: "aah | pilot | power (rt)"
author: "Owen Morgan"
...
```
<br>
30.088 - 3.654 = 26.434ms difference.
...
#rt_ehi_model |> summary()
## The slope of the effect of EHI on field x level is: .13
...
if (use_cached_sims == FALSE) {
  ...
  saveRDS(rt_sim, here(cache_dir, "rt_ehi_sim.rds"))
  saveRDS(rt_model_sim, here(cache_dir, "rt_ehi_model_sim.rds"))
...
if (use_cached_sims == FALSE) {
  ...
  rt_model_sim_extended <-
    extend(rt_model_sim, along = "subject", n = 606)
...
```{r simulate_lefties_rt_continuous}
rt_ehi_model <- lmer(rt ~ field*level*ehi + (1 | subject), data = aah_for_rt_model)
# summary(rt_ehi_model)
...
if (use_cached_sims == FALSE) {
  ...
  ## Make simulated lefties with the same RT data as righties:
  rt_ehi_sim_lefties <- rt_data_pilot |> mutate(ehi = -ehi)
  
  ## Make a dataset that includes the simulated lefties.
  rt_ehi_sim <- add_row(rt_data_pilot, rt_sim_lefties) |>
    mutate(handedness = case_when(ehi > 0 ~ "right",
                                  ehi <= 0 ~ "left"))
  
  ## Make a model using the simulated data
  rt_ehi_model_sim <-
    lmer(rt ~ field*level*ehi + (1 | subject), data = rt_ehi_sim)
  
  ## Manually cache dataset and model
  saveRDS(rt_ehi_sim, here(cache_dir, "rt_ehi_sim.rds"))
  saveRDS(rt_ehi_model_sim, here(cache_dir, "rt_ehi_model_sim.rds"))
  
} else if (use_cached_sims == TRUE) {
  ## Load cached model
  rt_ehi_sim <- readRDS(here(cache_dir, "rt_ehi_sim.rds"))
...
Calculate power to detect an interaction of EHI x field x level if the slope of EHI's effect on the field x level interaction is .132 (the slope estimated in the pilot sample of right handers).
...
if (use_cached_sims == FALSE) {
  ...
  rt_ehi_model_sim_extended <-
    extend(rt_ehi_model_sim, along = "subject", n = 606)
  #p2 <- powerSim(rt_model_sim_2, nsim = n_sim, test = compare(rt ~ field*level + (1 | subject)))
  ## nsim = 10, 100% power. nrow = 199568 (24946*4)
...
rt_ehi_pc_tbl |>
  pretty_table() |> tab_header(title = "Power to detect field x level x handedness (continuous) interaction", subtitle =  "alpha =  .05") |>
  tab_footnote(footnote = "Total subjects: 1/2 right, 1/2 left handed.",
               locations = cells_column_labels(columns = n_subjects)) |>
  tab_footnote(footnote = "Number of simulations where the interaction was significant, assuming the slope estimate we saw in right handers.",
               locations = cells_column_labels(columns = successes)
  )
