...
if (use_cached_sims == FALSE) {
  ...
  rt_model_sim_extended_2 <-
    extend(rt_model_sim_2, along = "subject", n = 606)
  #rt_model_sim_extended |> summary()
  #p2 <- powerSim(rt_model_sim, nsim = 5, test = compare(rt ~ field*level + (1 | subject)))
...
if (use_cached_sims == FALSE) {
  ...
  rt_ehi_model_sim_extended_2 <-
    extend(rt_ehi_model_sim_2, along = "subject", n = 606)
...
## DEBUGGING -- ARE RESULTS TOO GOOD TO BE TRUE?
## (1) What is the power to detect the effect in our sample of right handers?
# rt_ehi_model |> summary()
# p1 <- powerSim(rt_ehi_model, nsim = 100, test = compare(rt ~ field*level + (1 | subject)))
# p1
## With 100 simulations, 12% power.
# rt_pc_pilot <- powerCurve(
#     fit = rt_ehi_model |> extend(along = "subject", n = 406),
#     test = compare(rt ~ field * level + (1 |
#                                            subject)),
#     alpha = 0.05,
#     along = "subject",
...
#   )
# rt_pc_pilot
## (2) What is the power to detect the effect in a sample of 103 right, 103 left handers?
# p2 <- powerSim(rt_ehi_model_sim, nsim = 100, test = compare(rt ~ field*level + (1 | subject)))
# p2
## With 100 simulations, 90% power.
## This seems about right.
# rt_pc_pilot_sim <- powerCurve(
#     fit = rt_ehi_model_sim |> extend(along = "subject", n = 406),
#     test = compare(rt ~ field * level + (1 |
#                                            subject)),
#     alpha = 0.05,
#     along = "subject",
