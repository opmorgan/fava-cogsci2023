---
title: "Action Asymmetry Experiment (n = 672): Field by Level by Handedness Analyses"
...
```{r config}
proc_dir <- here::here("data", "proc_exp_n672")
fig_dir <- here::here("figures", "exp_n672")
manual_cache_dir <- here::here("manual_cache", "exp_n672")
...
<br>
<br>
 ***Summary.*** For reaction time, the interaction trends in the predicted direction (9.79ms, 95%CI [-3.84, 23.41], p = .08, one-sided). I haven't run accuracy yet, but the effect will be close to zero (the point estimates are 1.96 for righties, 1.94 for lefties).
<br>
<br>
...
***Summary.*** For reaction time, the interaction trends in the predicted direction (.063ms per EHI unit, or 15.54ms for EHI = -100, 28.12ms for EHI = +100. I haven't figured out how to get confidence intervals around this estimate yet, but the one-sided p-value for an ANOVA model comparison is .056.I haven't run the accuracy stats yet, but the subject-level scatter plot looks strikingly flat.
