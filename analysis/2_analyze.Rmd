...
interaction_anova |>
  as_tibble() |>
  pretty_table() |> 
  tab_header(title = "ANOVA: compare models with vs. without interaction term (RT)") |> 
    tab_footnote(footnote = "I don't understand exactly what this p-value means",
...
rt_interaction_emm |>
  as_tibble() |>
  pretty_table() |>
  tab_header(title = "Field by level interaction effect estimate (RT)") |>
...
<br>
**glmer( correct ~ field + level + field:level + (1 | subject) )**
<br>
*Discussion question*: Should any variables not-of-interest be included as covariates? E.g., sex, target (circle/square), first block (z or slash), overall median rt (ms).
```{r acc_model}
...
interaction_anova |>
  as_tibble() |>
  pretty_table() |> 
  tab_header(title = "ANOVA: compare models with vs. without interaction term (Accuracy)") |> 
    tab_footnote(footnote = "I don't understand exactly what this p-value means",
               locations = cells_column_labels(
                 columns = `Pr(>Chisq)`
               ))
...
acc_interaction_emm |>
  as_tibble() |>
  pretty_table() |>
  tab_header(title = "Field by level interaction effect estimate (Accuracy)") |>
  ...
  tab_footnote(footnote = "I don't understand why df is 'Inf' here, or how to interpret df for a logistic regression.",
               locations = cells_column_labels(columns = df)) |>
  tab_footnote(footnote = "Confidence level: 95%",
