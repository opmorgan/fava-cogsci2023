...
task_summary <- load_and_summarize_proc(ind_input_dir, summary_output_dir, data_type = "task")
}
```

```{r}
## TODO: join survey data
##          Create "load_and_summarize_proc arms for ehi, demographics, end.
##          Then, join task and survey tables together, save.
resummarize_ehi_data <- TRUE
if (resummarize_ehi_data == TRUE) {
## Create summarized task data (saves to tsv as a side effect)
ehi_summary <- load_and_summarize_proc(ind_input_dir, summary_output_dir, data_type = "ehi")
}
...
demo_summary <- load_and_summarize_proc(ind_input_dir, summary_output_dir, data_type = "demographics")
}

resummarize_end_data <- FALSE
if (resummarize_end_data == TRUE) {
## Create summarized end question data (saves to tsv as a side effect)
end_summary <- load_and_summarize_proc(ind_input_dir, summary_output_dir, data_type = "end")
}
```
...
```{r}
## Load all individual summary files, combine them together.
## Save as a big summary file, with a wide row for each subject.
```
